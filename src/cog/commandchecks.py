    ...
    async def banned_users(self, ctx: commands.Context):
        ...
        if ban:
            ...
            if self.bot.owner_id == 214511018204725248:  # my ID; public bot
                outtext += "raise an issue at https://github.com/machineonamission/mediaforge/issues/new?assignees=" \
            ...
            if "options" in ctx.interaction.data:
                for arg in ctx.interaction.data["options"]:
                    for block in config.blocked_words:
                        if isinstance(arg["value"], str) and block.lower() in arg["value"].lower():
